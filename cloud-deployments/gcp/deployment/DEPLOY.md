...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create aiappsy-llm-deployment --config gcp/deployment/gcp_deploy_anything_llm.yaml
...
```
gcloud compute instances get-serial-port-output aiappsy-llm-instance 
```

ssh into the instance

```
gcloud compute ssh aiappsy-llm-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete aiappsy-llm-deployment 
```
