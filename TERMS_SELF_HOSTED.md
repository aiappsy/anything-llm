# AiAppsy LLM Self-Hosted: Data Privacy & Terms of Service

This document outlines the privacy standards, data handling procedures, and licensing terms for the self-hosted version of AiAppsy LLM, developed by Mintplex Labs Inc.

## 1. Data Sovereignty & Local-First Architecture
AiAppsy LLM is designed as a **local-first** application. When utilizing the self-hosted version (Docker, Desktop, or Source):
...
* **Air-Gap Capability:** AiAppsy LLM can be operated in a strictly air-gapped environment with no internet connectivity, provided local LLM and Vector database providers (e.g., Ollama, LocalAI, LanceDB) are utilized.

## 2. Telemetry and Analytics
To improve software performance and stability, AiAppsy LLM includes an optional telemetry feature.
...
AiAppsy LLM allows users to connect to external services (e.g., OpenAI, Anthropic, Pinecone). 
...
_by default, AiAppsy LLM does **everything on-device first** - so you would have to manually configure and enable these integrations to be subject to third party terms._

## 4. Security & Network
* **No "Phone Home":** Aside from [optional telemetry](https://github.com/Mintplex-Labs/aiappsy-llm?tab=readme-ov-file#telemetry--privacy), the software does not require an external connection to Mintplex Labs servers to function.
* **Environment Security:** The user is responsible for securing the host environment, including network firewalls, SSL/TLS encryption, and access control for the AiAppsy LLM instance.
...
* **License:** The AiAppsy LLM core is provided under the **MIT License**.
