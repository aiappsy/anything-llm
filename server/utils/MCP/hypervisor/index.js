...
/**
 * @class MCPHypervisor
 * @description A class that manages MCP servers found in the storage/plugins/AIAPPSY_LLM_mcp_servers.json file.
 ...
 * For example, if the tools requires `npx` then the context in which AIAPPSY_LLM mains process is running will need to access npx.
 * This is typically not common in our pre-built image so may not function. But this is the case anywhere MCP is used.
 *
 * AIAPPSY_LLM will take care of porting MCP servers to agent-callable functions via @agent directive.
...
class MCPHypervisor {
  ...
  #setupConfigFile() {
    this.mcpServerJSONPath =
      process.env.NODE_ENV === "development"
        ? path.resolve(
            __dirname,
            `../../../storage/plugins/AIAPPSY_LLM_mcp_servers.json`
          )
        : path.resolve(
            process.env.STORAGE_DIR ??
              path.resolve(__dirname, `../../../storage`),
            `plugins/AIAPPSY_LLM_mcp_servers.json`
          );
  ...
  updateSuppressedTools(serverName, toolName, enabled) {
    ...
    if (!server.AIAPPSY_LLM) server.AIAPPSY_LLM = {};
    if (!Array.isArray(server.AIAPPSY_LLM.suppressedTools))
      server.AIAPPSY_LLM.suppressedTools = [];

    const suppressedTools = server.AIAPPSY_LLM.suppressedTools;
    ...
    server.AIAPPSY_LLM.suppressedTools = suppressedTools;
  ...
  getSuppressedTools(serverName) {
    ...
    return config?.server?.AIAPPSY_LLM?.suppressedTools || [];
  }
  ...
  async #buildMCPServerENV(server) {
    ...
    if (process.env.AIAPPSY_LLM_RUNTIME === "docker") {
      baseEnv = {
  ...
  async bootMCPServers() {
    ...
    for (const { name, server } of serverDefinitions) {
      if (
        server.AIAPPSY_LLM?.hasOwnProperty("autoStart") &&
        server.AIAPPSY_LLM.autoStart === false
      ) {
        this.log(
          `MCP server ${name} has AIAPPSY_LLM.autoStart property set to false, skipping boot!`
        );
        this.mcpLoadingResults[name] = {
          status: "failed",
          message: `MCP server ${name} has AIAPPSY_LLM.autoStart property set to false, boot skipped!`,
        };
        continue;
      }
