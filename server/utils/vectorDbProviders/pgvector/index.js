...
class PGVector extends VectorDatabase {
  ...
  async validateExistingEmbeddingTableSchema(pgClient, tableName) {
    ...
    if (result.rows.length === 0)
      throw new Error(
        `The table '${tableName}' was found but does not contain any columns or cannot be accessed by role. It cannot be used as an embedding table in AiAppsy LLM.`
      );
   ...
   * so that AiAppsy LLM can manage the table directly.
   *
  ...
  /**
   * Check if the AiAppsy LLM embedding table exists in the database
   * @returns {Promise<boolean>}
   */
  async dbTableExists() {
    let connection = null;
  ...
  /**
   * Reset the entire vector database table associated with AiAppsy LLM
