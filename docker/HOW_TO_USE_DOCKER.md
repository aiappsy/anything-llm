...
Use the Dockerized version of AiAppsy LLM for a much faster and complete startup of AiAppsy LLM.

### Minimum Requirements

> [!TIP]
> Running AiAppsy LLM on AWS/GCP/Azure?
...
\*AiAppsy LLM by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*AiAppsy LLM by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized AiAppsy LLM!
...
> the docker container using AiAppsy LLM as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/AiAppsy LLM
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run AiAppsy LLM in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/AiAppsy LLM && \
...
$env:STORAGE_LOCATION="$HOME\Documents\AiAppsy LLM"; `
...
services:
  AiAppsy LLM:
    image: mintplexlabs/AiAppsy LLM
    container_name: AiAppsy LLM
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - AiAppsy LLM_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using AiAppsy LLM! All your data and progress will persist between
...
- `git clone` this repo and `cd aiappsy-llm` to get to the root directory.
- `touch server/storage/AiAppsy LLM.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of AiAppsy LLM easier.

### Use the Midori AI Subsystem to Manage AiAppsy LLM

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the AiAppsy LLM docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in AiAppsy LLM.
